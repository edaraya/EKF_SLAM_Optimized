...
#include "ekf_logger.hpp"
#include "ekf_fpga.hpp"
  ...
  void fake_sensor_cb(const visualization_msgs::msg::MarkerArray & msg)
  {
    ...
    for (size_t i = 0; i < fake_sensor_data_.markers.size(); i++) {
      ...
      if (marker.action != visualization_msgs::msg::Marker::DELETE) {
        ...
        // eq 26
        arma::mat PHt = fpga_.compute_PHt(sigma, Hj);
        logger_.record_fpga_times(fpga_.last_sync_to_us,
                                  fpga_.last_kernel_us,
                                  fpga_.last_sync_from_us);
    ...
    logger_.record_state(state, sigma);
